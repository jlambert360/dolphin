...
PlayerFrameData CreateDummyPlayerFrameData(u32 frame, u8 playerIdx) {
    PlayerFrameData dummyframe = PlayerFrameData();
    dummyframe.frame = frame;
    dummyframe.playerIdx = playerIdx;
    dummyframe.pad = BrawlbackPad();  // empty pad
    return dummyframe;
}
...
void CEXIBrawlback::handleLocalPadData(u8* data)
{
    ...
    if (frame == GAME_START_FRAME) {
        ...
        for (int i = GAME_START_FRAME; i < FRAME_DELAY; i++) {
            this->remotePlayerFrameData[playerIdx].push_back(std::make_unique<PlayerFrameData>(CreateDummyPlayerFrameData(i, playerIdx)));
            this->localPlayerFrameData.push_back(std::make_unique<PlayerFrameData>(CreateDummyPlayerFrameData(i, playerIdx)));
        }
    ...
    if (shouldTimeSync) {
        ...
        if (this->rollbackInfo.pastFrameDataPopulated) {
            ...
            Reset(this->rollbackInfo);  // reset rollbackInfo
            /*for (FrameData fd : rollbackInfo.pastFrameDatas) {
...
void CEXIBrawlback::storeLocalInputs(PlayerFrameData* localPlayerFramedata) {
    ...
    if (this->localPlayerFrameData.size() + 1 > FRAMEDATA_MAX_QUEUE_SIZE) {
        ...
        PlayerFrameData* pop_data = this->localPlayerFrameData.front().release();
...
std::pair<bool, bool> CEXIBrawlback::getInputsForGame(FrameData& framedataToSendToGame, u32 frame) {
    ...
    for (int playerIdx = 0; playerIdx < this->numPlayers; playerIdx++) {
        ...
        if (!this->remotePlayerFrameData.empty() && !this->remotePlayerFrameData[playerIdx].empty()) {
            ...
            PlayerFrameData* remoteFrameData = findInPlayerFrameDataQueue(this->remotePlayerFrameData[playerIdx], frame);
                ...
                if (!this->rollbackInfo.isUsingPredictedInputs && frame > GAME_FULL_START_FRAME) {
                    ...
                    for (u32 frameIter = frameWithDelay; frameIter >= searchEndFrame; frameIter--) {
                        ...
                        if (this->remotePlayerFrameDataMap[playerIdx].count(frameIter)) {
                            ...
                            PlayerFrameData mostRecentFramedata = *this->remotePlayerFrameDataMap[playerIdx][frameIter];
                            ...
                            PlayerFrameData pastLocalInputs = *this->localPlayerFrameData[this->localPlayerFrameData.size()-1 -FRAME_DELAY].get();
                    ...
                    predictedInputs = this->rollbackInfo.predictedInputs.playerFrameDatas[playerIdx];
                    ...
                    if (numFramesWithoutRemoteInputs <= MAX_ROLLBACK_FRAMES) {
                        ...
                        PlayerFrameData* localInputs = findInPlayerFrameDataQueue(this->localPlayerFrameData, pastLocalInputDesiredFrame);
                        if (localInputs) {
...
void CEXIBrawlback::SetupRollback(u32 frame) {
        ...
        u32 frameDiff = i - this->rollbackInfo.beginFrame;
            ...
            ret = std::make_optional(*predictedInputs);
                ...
                if (i == this->rollbackInfo.endFrame) { 
                    PlayerFrameData* pastFramedata = findInPlayerFrameDataQueue(this->localPlayerFrameData, i);
                    if (pastFramedata) {
                        memcpy(&this->rollbackInfo.pastFrameDatas[frameDiff].playerFrameDatas[pIdx], pastFramedata, sizeof(PlayerFrameData));
    ...
    return ret;
                ...
                PlayerFrameData* pastFramedata = this->remotePlayerFrameDataMap[pIdx][i];
                memcpy(&this->rollbackInfo.pastFrameDatas[frameDiff].playerFrameDatas[pIdx], pastFramedata, sizeof(PlayerFrameData));
...
void CEXIBrawlback::ProcessIndividualRemoteFrameData(PlayerFrameData* framedata) {
    ...
    while (remoteFramedataQueue.size() > FRAMEDATA_MAX_QUEUE_SIZE) {
        ...
        PlayerFrameData* front_data = remoteFramedataQueue.front().release();
